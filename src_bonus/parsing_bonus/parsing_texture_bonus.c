...
/*   Updated: 2024/06/01 20:13:50 by dvo              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	convert_char_color(t_cub *cub, char *str)
{
	...
	third = ft_atoi(str + i);
	free (str);
	check_overflow_rgb(cub, first, second, third);
...
int	direc1(int direc, char *str, t_cub *cub, int i)
{
	if (direc == 1)
	{
		if (cub->map->north)
		{
			ft_printf(2, "Error\nDuplicate data for texture North\n");
			return (-1);
		}
	...
	if (direc == 2)
	{
		if (cub->map->south)
		{
			ft_printf(2, "Error\nDuplicate data for texture South\n");
			return (-1);
		}
...
int	direc2(int direc, char *str, t_cub *cub, int i)
{
	if (direc == 3)
	{
		if (cub->map->west)
		{
			ft_printf(2, "Error\nDuplicate data for texture West\n");
			return (-1);
		}
	...
	if (direc == 4)
	{
		if (cub->map->east)
		{
			ft_printf(2, "Error\nDuplicate data for texture East\n");
			return (-1);
		}
		cub->map->east = ft_strtrim(str + i, " ");
		if (!cub->map->east)
			return (-1);
	}
	return (0);
