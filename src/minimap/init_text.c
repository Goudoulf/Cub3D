...
{
	cub->mini_map.n_bot = ft_calloc(1, sizeof(t_tex));
	cub->mini_map.n_bot->img_ptr = mlx_xpm_file_to_image(cub->mlx, "./textures/hero/neutral_b.xpm", &cub->mini_map.n_bot->width, &cub->mini_map.n_bot->height);	
	if (!cub->mini_map.n_bot->img_ptr)
		return (-1);
	if (resize_image(cub, cub->mini_map.n_bot, cub->mini_map.x_case, cub->mini_map.y_case) == -1)
		return (-1);
	cub->mini_map.n_bot->buffer = mlx_get_data_addr(cub->mini_map.n_bot->img_ptr,
			&cub->mini_map.n_bot->bpp, &cub->mini_map.n_bot->line_len, &cub->mini_map.n_bot->endian);
	return (0);
}
