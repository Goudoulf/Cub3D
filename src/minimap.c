...
void	ft_set_minimap(t_cub *cub)
{
	...
	y = 0;
	wall = (0 << 16) | (255 << 8) | 0;
	border = (218 << 16) | (165 << 8) | 32;
	while (y < cub->win_y / 5)
	{
		while (x < cub->win_x / 5)
		{
			offset = (cub->mini_map.img.line_len * y)
				+ (x * (cub->mini_map.img.bpp / 8));
			if (ft_border(cub, x, y))
				...
				*((unsigned int *)(offset + cub->mini_map.img.buffer))
					= cub->map->floor;
			x++;
		}
		y++;
...
{
	cub->mini_map.img.img_ptr
		= mlx_new_image(cub->mlx, cub->win_x / 5, cub->win_y / 5);
	...
	cub->mini_map.img.buffer
		= mlx_get_data_addr(cub->mini_map.img.img_ptr,
