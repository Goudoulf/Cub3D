...
/*	 Updated: 2024/05/29 14:11:28 by cassie			  ###	########.fr		  */
/*																			  */
/* ************************************************************************** */
...
void	ft_ennemy_move(t_cub *cub)
{
	...
	if (cub->view_ennemy == 1
		...
		return ;
	cub->fps_statue = cub->view_ennemy == 1
		&& (tv.tv_sec * 1000000 + tv.tv_usec) + 1000000;
...
void ray_draw_statue(t_cub *cub, int x, int h)
{
	...
	if (cub->ray->statue.side == 0 && cub->ray->vector.x > 0) 
	texX = 64 - texX - 1;
	if (cub->ray->statue.side == 1 && cub->ray->vector.y < 0) 
	texX = 64 - texX - 1;
	if (cub->ray->statue.side == 0 && cub->ray->vector.x < 0) 
	tex = &cub->texture.statue;
	if (cub->ray->statue.side == 1 && cub->ray->vector.y < 0) 
	tex = &cub->texture.statue;
	if (cub->ray->statue.side == 0 && cub->ray->vector.x > 0) 
	tex = &cub->texture.statue;
	...
	{
		int texY = (int)(drawStart * 2 - h + lineHeight)
		* ((64 / 2.) / lineHeight);
